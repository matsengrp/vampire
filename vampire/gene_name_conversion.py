...
def adaptive_to_olga_dict():
    ...
    return {
        locus: {row["adaptive"]: row["olga"] for _, row in df.iterrows()}
        for locus, df in gb
    }
...
def olga_to_adaptive_dict():
    gb = common.read_data_csv(translation_csv).dropna().groupby("locus")
    return {
        locus: {row["olga"]: row["adaptive"] for _, row in df.iterrows()}
        for locus, df in gb
    }
...
def filter_by_gene_names(df, conversion_dict):
    ...
    return df.loc[
        df["v_gene"].isin(allowed["TRBV"]) & df["j_gene"].isin(allowed["TRBJ"]), :
    ]
...
def convert_and_filter(df, conversion_dict):
    orig_len = len(df)
    converted = convert_gene_names(
        filter_by_gene_names(df, conversion_dict), conversion_dict
    )
    n_trimmed = orig_len - len(converted)
    if n_trimmed > 0:
        click.echo(
            f"Warning: couldn't convert {n_trimmed} sequences and trimmed them off."
        )
    return converted
...
def adaptive2olga(adaptive_csv, olga_tsv):
    ...
    convert_and_filter(df, adaptive_to_olga_dict()).to_csv(
        olga_tsv, sep="\t", index=False, header=False
    )
