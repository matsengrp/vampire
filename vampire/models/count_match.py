...
from vampire.custom_keras import (
    BetaWarmup,
    CDR3Length,
    ContiguousMatch,
    EmbedViaMatrix,
...
def build(params):
    ...
    def sampling(args):
        ...
        epsilon = K.random_normal(
            shape=(params["batch_size"], params["latent_dim"]), mean=0.0, stddev=1.0
        )
    ...
    # Encoding layers:
    cdr3_embedding = EmbedViaMatrix(params["aa_embedding_dim"], name="cdr3_embedding")(
        cdr3_input
    )
    cdr3_embedding_flat = Reshape(
        [params["aa_embedding_dim"] * params["max_cdr3_len"]],
        name="cdr3_embedding_flat",
    )(cdr3_embedding)
    v_gene_embedding = Dense(params["v_gene_embedding_dim"], name="v_gene_embedding")(
        v_gene_input
    )
    j_gene_embedding = Dense(params["j_gene_embedding_dim"], name="j_gene_embedding")(
        j_gene_input
    )
    merged_embedding = keras.layers.concatenate(
        [
            ...
            v_gene_embedding,
            j_gene_embedding,
            contiguous_match_input,
        ],
        name="merged_embedding",
    )
    encoder_dense_1 = Dense(
        params["dense_nodes"], activation="elu", name="encoder_dense_1"
    )(merged_embedding)
    encoder_dense_2 = Dense(
        params["dense_nodes"], activation="elu", name="encoder_dense_2"
    )(encoder_dense_1)
    ...
    decoder_dense_1_l = Dense(
        params["dense_nodes"], activation="elu", name="decoder_dense_1"
    )
    decoder_dense_2_l = Dense(
        params["dense_nodes"], activation="elu", name="decoder_dense_2"
    )
    cdr3_post_dense_flat_l = Dense(
        np.array(cdr3_input_shape).prod(),
        activation="linear",
        name="cdr3_post_dense_flat",
    )
    ...
    v_gene_output_l = Dense(
        params["n_v_genes"], activation="softmax", name="v_gene_output"
    )
    j_gene_output_l = Dense(
        params["n_j_genes"], activation="softmax", name="j_gene_output"
    )
    ...
    germline_cdr3_tensors = conversion.adaptive_aa_encoding_tensors(
        params["max_cdr3_len"]
    )
    (v_germline_cdr3_tensor, j_germline_cdr3_tensor) = germline_cdr3_tensors
    (v_max_germline_aas, j_max_germline_aas) = [
        max_germline_aas(g) for g in germline_cdr3_tensors
    ]
    v_germline_cdr3_l = RightTensordot(
        v_germline_cdr3_tensor, axes=1, name="v_germline_cdr3"
    )
    j_germline_cdr3_l = RightTensordot(
        j_germline_cdr3_tensor, axes=1, name="j_germline_cdr3"
    )
    cdr3_length_output_l = CDR3Length(name="cdr3_length_output")
    contiguous_match_output_l = ContiguousMatch(
        v_max_germline_aas, j_max_germline_aas, name="contiguous_match_output"
    )
    ...
    contiguous_match_output = contiguous_match_output_l(
        [cdr3_output, v_germline_cdr3, j_germline_cdr3]
    )
        ...
        Add(name="cdr3_pre_activation")(
            [
                ...
                Add(name="germline_cdr3")(
                    [decoder_v_germline_cdr3, decoder_j_germline_cdr3]
                ),
            ]
        )
    )
    ...
    decoder = Model(
        ...
        [
            decoder_cdr3_output,
            decoder_cdr3_length_output,
            decoder_v_gene_output,
            decoder_j_gene_output,
...
def prepare_data(x_df):
    ...
    contiguous_match_data = conversion.contiguous_match_counts_df(
        x_df, v_germline_tensor, j_germline_tensor
    )
    return [
        cdr3_data,
        cdr3_length_data,
