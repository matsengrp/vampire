...
def plot_by_dirs(glob_str, d_name, **plot_kwargs):
    """
    Aggregate data from a numerically-indexed directory hierarchy nested in one
    indexed by data sets and make an array of plots.
    """
    dfs = []
    for path in glob.glob(glob_str):
        run_on, d = path.split('/')[-3:][0:2]
        df = pd.read_csv(path, index_col=0).T
        df.index.names = ['train_vs_test']
        for k, v in [('run_on', run_on), (d_name, float(d))]:
            df[k] = v
    ...
    for i, (train_vs_test, train_vs_test_df) in enumerate(df.groupby(level=0)):
        for j, (run_on, run_on_df) in enumerate(train_vs_test_df.groupby(level=1)):
            to_plot = run_on_df[['loss']]
            to_plot.index = to_plot.index.droplevel([0, 1])
            to_plot.plot(ax=axs[i, j], **plot_kwargs)
