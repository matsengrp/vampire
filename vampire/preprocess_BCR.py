...
def filter_on_cdr3_bounding_aas(df):
    ...
    the CDR3 amino acid.
    """
    return df[df['amino_acid'].str.contains('^C.*W$')]
...
def filter_on_IGH(df):
    """
    Only take sequences that have a resolved IGH gene for V and J.
    """
    return df[df['v_gene'].str.contains('^IGH') & df['j_gene'].str.contains('^IGH')]



def apply_all_filters(df, max_len=40):
    """
    Apply all filters.
    """
