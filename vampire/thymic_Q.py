...
def q_of_train_and_model_pgen(
    model_p_lvj_csv, train_tsv, max_q=None, pseudocount_multiplier=0.5
):
    """
    ...
    df = merge_lvj_dfs(
        lvj_frequency_of_olga_tsv(train_tsv, col_name="data_P_lvj"),
        pd.read_csv(model_p_lvj_csv, index_col=lvj),
    )
...
def sample_Ppost(sample_size, q_csv, max_iter=100, proposal_size=1e6):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for iter_idx in range(max_iter):
            ...
            c = delegator.run(
                " ".join(["olga-generate.sh", str(proposal_size), sample_path])
            )
            if c.return_code != 0:
    ...
    if len(out_df) < sample_size:
        raise Exception(
            "Did not obtain desired number of samples in the specified maximumn number of iterations."
        )
...
@click.option(
    ...
    default=None,
    show_default=True,
    help="Limit q to be at most this value.",
)
...
@click.option(
    ...
    default=1000000,
    show_default=True,
    help="Number of samples to take for proposal distribution.",
)
...
def sample(max_iter, proposal_size, sample_size, q_csv, out_tsv):
    ...
    """
    sample_Ppost(
        sample_size, q_csv, max_iter=max_iter, proposal_size=proposal_size
